...
function dependencies(overrides: Partial<CoachChatDependencies> = {}): CoachChatDependencies {
  ...
  return {
    ...
    loadSession: jest.fn(async () => ({ id: sessionId, userId, status: "complete", videoPath: `${userId}/${sessionId}/original.mp4`, result: { score: 82 } })),
...
describe("coach chat handler", () => {
  ...
  it("persists the user first, grounds generation, then saves the assistant", async () => {
    ...
    expect(deps.generateReply).toHaveBeenCalledWith(expect.objectContaining({ videoFile: expect.objectContaining({ state: "ACTIVE" }), analysis: { score: 82 }, history: expect.arrayContaining([expect.objectContaining({ role: "user" })]) }));
