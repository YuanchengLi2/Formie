...
function normalizeFindings(findings: AnalysisCandidate["priorityCorrections"], requireActionable = false): AnalysisCandidate["priorityCorrections"] {
  return findings.map((finding) => ({
    ...finding,
    coachingType: finding.coachingType ?? "correction",
    observedIssueRegions: (Array.isArray(finding.observedIssueRegions) ? finding.observedIssueRegions : []).filter((region): region is NonNullable<typeof finding.observedIssueRegions>[number] => typeof region === "string" && ANATOMY_REGIONS.has(region)),
    actionableCorrection: finding.actionableCorrection ?? (finding.correction || finding.cue || requireActionable ? {
      instruction: finding.correction ?? finding.cue ?? finding.title,
      cue: finding.cue ?? finding.correction ?? finding.title,
      successCheck: finding.cue ?? finding.correction ?? finding.title,
    ...
    primaryEvidenceIndex: Number.isInteger(finding.primaryEvidenceIndex) && Number(finding.primaryEvidenceIndex) >= 0 && Number(finding.primaryEvidenceIndex) < finding.evidence.length ? Number(finding.primaryEvidenceIndex) : 0,
  }));
}
...
function normalizeEquipmentObservations(value: unknown): AnalysisCandidate["equipmentObservations"] {
  ...
  return value.flatMap((item) => {
    ...
    return [{ ...observation, load: normalizeEquipmentLoad(observation.load, observation) } as AnalysisCandidate["equipmentObservations"][number]];
  });
}

export function historicalResultPayload(session: Record<string, unknown>, result: Record<string, unknown> | null): AnalysisCandidate | null {
  ...
  // Historical output is made structurally readable only. Its language is not
  // re-evaluated or rewritten by the retired coaching sanitizer.
  const priorityCorrections = normalizeFindings((result.priority_corrections ?? []) as AnalysisCandidate["priorityCorrections"], true);
  ...
  try { declaration = session.set_declaration ? parseSetDeclaration(session.set_declaration) : null; } catch { declaration = null; }
  ...
  const validPersistedPlan = persistedPlan.filter((plan) => plan.relatedFindingId !== null && correctionIds.has(plan.relatedFindingId));
  const firstCorrection = priorityCorrections[0] ?? null;
  const nextSetPlan = validPersistedPlan.length > 0 ? validPersistedPlan : firstCorrection ? [{ id: "legacy-next-set", action: firstCorrection.correction ?? firstCorrection.cue ?? firstCorrection.title, rationale: firstCorrection.whyItMatters, successCheck: firstCorrection.actionableCorrection?.successCheck ?? "The cited deviation is no longer visible.", relatedFindingId: firstCorrection.id }] : [];
  const hasPersistedScore = result.score !== null && result.score !== undefined && Number.isFinite(Number(result.score));
  const rawScore = status === "unable" ? null : hasPersistedScore ? Math.max(0, Math.min(100, Number(result.score))) : 75;
  const score = rawScore === null ? null : hasPersistedScore ? calibratedTechniqueScore(rawScore, priorityCorrections) : rawScore;
  return {
    status,
    recognition: { label: declaration?.exercise.label ?? (session.corrected_label ?? session.detected_label ?? null) as string | null, variation: (session.detected_variation ?? null) as string | null, equipment: (session.detected_equipment ?? []) as string[], confidence: declaration ? 1 : Number(session.recognition_confidence ?? 0), alternatives: declaration ? [] : (session.recognition_alternatives ?? []) as string[], catalogExerciseId: declaration?.exercise.catalogExerciseId ?? (session.corrected_exercise_id ?? session.exercise_variant_v2_id ?? session.exercise_id ?? null) as number | null, exerciseFamily: (session.exercise_family ?? "other") as AnalysisCandidate["recognition"]["exerciseFamily"], source: declaration ? "user_declared" : "legacy_model" },
    ...
    coachNote: typeof result.coach_note === "string" && result.coach_note.trim() ? result.coach_note.trim() : null,
    score,
    scoreRationale: (result.score_rationale ?? []) as AnalysisCandidate["scoreRationale"],
    movementScores: Array.isArray(result.movement_scores) && result.movement_scores.length === 4 ? result.movement_scores as NonNullable<AnalysisCandidate["movementScores"]> : [],
    ...
    ...(result.exercise_guide && typeof result.exercise_guide === "object" ? { exerciseGuide: result.exercise_guide as AnalysisCandidate["exerciseGuide"] } : {}),
    ...(Array.isArray(result.coaching_coverage) && result.coaching_coverage.length > 0 ? { coachingCoverage: result.coaching_coverage as NonNullable<AnalysisCandidate["coachingCoverage"]> } : {}),
    didWell,
    priorityCorrections,
    coachingCues,
    ...
    setSummary: { ...((result.set_summary ?? { totalReps: null, consistentReps: null, verdict: null }) as AnalysisCandidate["setSummary"]), ...(declaration?.amount.kind === "reps" ? { totalReps: declaration.amount.value } : {}) },
