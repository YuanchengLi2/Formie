...
When supplemental movement estimates are present, they come from MoveNet Thunder and are supplemental 2D estimates only. The original video remains authoritative. Coordinates are normalized: x runs from 0 at image-left to 1 at image-right, and y runs from 0 at image-top to 1 at image-bottom. Height-delta columns are signed left-y minus right-y, bend columns are two-dimensional degrees, and confidence describes landmark visibility. Treat changes across several reliable frames as stronger evidence than one isolated estimate. Use the timestamped series to locate possible rep transitions, joint-path changes, range changes, or left/right differences, then verify every coaching claim in the original pixels. Low-confidence or missing landmarks are not technique faults; never infer depth, pain, muscle activation, or hidden joint positions from these estimates. Use original-video milliseconds for final evidence.
