...
describe("resultPayload", () => {
  it("maps persisted Gemini recognition and coaching into the app contract", () => {
    expect(resultPayload(
      ...
      { status: "complete", video_check: { outcome: "usable", usableObservations: [], limitations: [], retryReason: null, retryInstruction: null }, overall_assessment: "Controlled", score: null, score_rationale: [], did_well: [], priority_corrections: [], coaching_cues: [], set_summary: { totalReps: 8, consistentReps: 7, verdict: "Seven stayed controlled." }, rep_timeline: [{ repNumber: 1, startMs: 100, peakMs: 500, endMs: 900, assessment: "strong", note: "Controlled" }], next_set_plan: [{ id: "plan-1", action: "Repeat the tempo", rationale: "Keep control", relatedFindingId: null }], verification: { performed: true, reason: "subtle", outcome: "confirmed", checkedFindingId: "finding-1" }, comparison: null },
    )).toMatchObject({
