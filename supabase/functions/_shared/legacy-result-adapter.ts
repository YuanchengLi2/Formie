...
import { normalizeMuscleFocus } from "./result-compatibility.ts";
...
export function legacyResultPayload(session: Record<string, unknown>, result: Record<string, unknown> | null): AnalysisCandidate | null {
  ...
  return {
    ...
    movementScores: Array.isArray(result.movement_scores) && result.movement_scores.length === 4
      ? result.movement_scores as NonNullable<AnalysisCandidate["movementScores"]>
      : [],
