-- Development-only Test Store controls.  RevenueCat's Test Store cadence is
-- fixed, so the 20-minute period used by Formie is simulated in this table.
alter table public.subscription_test_scenarios
  add column if not exists quota_remaining_override integer,
  add column if not exists quota_actual_used_at_override integer,
  add column if not exists quota_override_period_start timestamptz,
  add column if not exists quota_override_period_end timestamptz;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'subscription_test_scenarios_quota_remaining_check') then
    alter table public.subscription_test_scenarios add constraint subscription_test_scenarios_quota_remaining_check check (quota_remaining_override is null or quota_remaining_override between 0 and 10);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'subscription_test_scenarios_quota_actual_used_check') then
    alter table public.subscription_test_scenarios add constraint subscription_test_scenarios_quota_actual_used_check check (quota_actual_used_at_override is null or quota_actual_used_at_override >= 0);
  end if;
end;
$$;
...
begin
  ...
  -- A scenario is only allowed to override the matching Test Store product.
  -- This prevents an expired monthly scenario from masking a repurchase or an
  -- annual upgrade that RevenueCat has already recorded.
