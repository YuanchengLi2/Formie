-- Preserve the distinction between an expired subscription and an account that
-- has never subscribed. The normalized status remains `expired` for client
-- compatibility while lifecycle_state carries `not_subscribed`.
