-- Search ordinary exercises by default. Generated execution-style variants
-- are eligible only when the user explicitly asks for a style modifier.
...
as $$
  ...
  query_flags as (
    select
      expanded.*,
      (
        raw_query ~ '(^| )(tempo|pause|paused|lengthened|shortened|partial|constant|tension|dead stop|slow concentric|one and a half)( |$)'
      ...
      and (
        ...
        or (
          ...
          and (
            ...
            or (query_flags.slow_concentric_requested and variant.execution_style = 'slow-concentric')
