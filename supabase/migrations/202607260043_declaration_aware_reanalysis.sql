drop function if exists public.reset_analysis_for_reanalysis(uuid, uuid);
...
comment on function public.reset_analysis_for_reanalysis(uuid, uuid, jsonb) is
  'Atomically requeues a saved upload while preserving or replacing its authoritative set declaration.';
