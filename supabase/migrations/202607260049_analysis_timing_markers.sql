...
comment on column public.analysis_sessions.upload_started_at is
  'Server timestamp for the first upload lifecycle stage.';
comment on column public.analysis_sessions.upload_completed_at is
  'Server timestamp after all required video objects are visible and finalization begins.';
comment on column public.analysis_sessions.analysis_started_at is
  'Server timestamp used with completed_at for tap-to-result and processing-duration benchmarks.';
...
comment on function public.reset_analysis_for_reanalysis(uuid, uuid, jsonb) is
  'Atomically requeues a saved upload with fresh result, retry, telemetry, and analysis timing state while preserving the saved video and declaration.';
