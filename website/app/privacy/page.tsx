...
export default function PrivacyPage() {
  return (
    <LegalPage eyebrow="Your privacy" title="Privacy Policy" updated="September 5, 2026">
      <p>This policy explains how Formie handles information when you use the app or website.</p>
      <h2>Information Formie handles</h2>
      <p>Formie handles your name, email, user ID, profile and fitness inputs, exercise declarations, purchase and entitlement status, exercise recordings you upload, support content, analysis results, any legacy coaching conversations created during earlier testing, creator referral attribution, product interaction analytics, and diagnostics such as app version, build, platform, and OS version. Before sign-in, analytics uses an installation-scoped random identity; it can be linked to your account after successful authentication. The Formie 1.0 Coach Preview does not let users create new conversations. In-app feedback never automatically includes a recording or analysis content.</p>
      ...
      <p>If you choose Affiliated creator during onboarding, Formie sends the creator code you enter to its own server to validate it. A valid code creates an opaque, expiring referral claim that can be attached only to the new account created during that onboarding attempt. The creator sees aggregate performance and a pseudonymous referral record, never your exercise activity or personal onboarding answers.</p>
      ...
      <p>Account, purchase, content, support, diagnostic, referral, and product interaction data may be linked to your Formie user ID. A self-reported acquisition answer stays separate from a verified creator referral and does not create a reward. Formie does not use this information for cross-company advertising tracking.</p>
      ...
      <p>Deleting your Formie account does not cancel your Apple subscription. Formie also requests deletion or revocation from Sign in with Apple, Google Gemini, RevenueCat, Supabase, and Formie storage. Provider failures are preserved in an encrypted cleanup queue so they do not block deletion of the Formie account. If a legacy Sign in with Apple account has no stored revocation credential, Formie still deletes the account and directs the user to Apple&apos;s settings to revoke Formie access manually. Public profile data, onboarding answers, and analytics identity links are deleted. Formie can retain an opaque account reference, financial ledger records, and the minimum receipt fingerprint needed to prevent duplicate referral rewards, along with limited security, fraud-prevention, backup, support, and legal records under their applicable schedules.</p>
      <h2>Security</h2>
