...
export function ManageSubscriptionClient({ initialDashboard, initialAuthenticated = false, initialError = null }: Props) {
  ...
  if (!dashboard) return <section className="account-portal signed-out"><div className="portal-intro"><span className="portal-kicker">FORMIE ACCOUNT</span><h1>Manage subscription</h1><p>Sign in with the same Apple or Google account you use in Formie.</p>{error ? <p className="portal-error" role="alert">{error}</p> : null}</div><div className="social-login"><button disabled={Boolean(busy)} onClick={() => void login("apple")}><Image className="provider-icon" src="/assets/apple-provider.png" width={32} height={32} alt="Apple" /><span>{busy === "apple" ? "Connecting..." : "Continue with Apple"}</span><i aria-hidden="true" /></button><button disabled={Boolean(busy)} onClick={() => void login("google")}><Image className="provider-icon" src="/assets/google-provider.png" width={32} height={32} alt="Google" /><span>{busy === "google" ? "Connecting..." : "Continue with Google"}</span><i aria-hidden="true" /></button></div></section>;
