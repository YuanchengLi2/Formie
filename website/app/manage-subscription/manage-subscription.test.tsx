...
test("signed-out portal offers Apple and Google with larger recognizable icons", () => { const html = renderToStaticMarkup(<ManageSubscriptionClient initialDashboard={null} />); assert.match(html, /Continue with Apple/); assert.match(html, /Continue with Google/); assert.match(html, /apple-provider\.png/); assert.match(html, /google-provider\.png/); assert.match(html, /width="32" height="32"/); assert.match(styles, /grid-template-columns:\s*32px 1fr 32px/); assert.match(styles, /\.provider-icon\s*\{[^}]*width:\s*32px;[^}]*height:\s*32px/); assert.doesNotMatch(html, /email.*input|password/i); });
