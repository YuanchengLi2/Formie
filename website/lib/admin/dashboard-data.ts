...
function metric(value: unknown, label: string, boundedRatio = false): MetricValue<number> {
  ...
  if (numerator !== null && denominator !== null && (numerator < 0 || denominator < 0 || (boundedRatio && numerator > denominator))) throw new Error(`Contradictory ${label}`);
...
function isBoundedRatioKey(key: string): boolean { return key.toLowerCase().includes("rate") || key === "wau_mau"; }
function metricRecord(value: unknown, label: string): Record<string, MetricValue<number>> { const row = object(value, label); return Object.fromEntries(Object.entries(row).map(([key, item]) => [key, metric(item, `${label}.${key}`, isBoundedRatioKey(key))])); }
...
export function parseDashboardSnapshot(input: unknown): AdminDashboardSnapshot {
  ...
  const headlineRaw = object(root.headline, "headline"); const headline = {} as HeadlineMetrics; for (const key of headlineKeys) headline[key] = metric(headlineRaw[key], `headline.${key}`, isBoundedRatioKey(key));
