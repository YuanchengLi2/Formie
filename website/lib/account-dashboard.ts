...
export function parseAccountDashboard(value: unknown): AccountDashboardResponse {
  ...
  const validState = ["active_renewing", "active_cancelled", "expired", "not_subscribed"].includes(candidate?.subscription?.state ?? "");
  ...
  const statesAgree = !candidate || (
    usage?.status === "active"
      ? candidate.subscription.state === "active_renewing" || candidate.subscription.state === "active_cancelled"
