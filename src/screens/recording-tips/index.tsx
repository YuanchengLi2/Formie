...
const CHECKLIST = ["Keep the full movement visible", "Use a side or diagonal view", "Use 0.5x if space is limited"] as const;

export function RecordingTipsScreen({ onContinue, onOpenSpaceHelp }: { onContinue: () => void; onOpenSpaceHelp: () => void }) {
  return (
    <ScrollView bounces={false} overScrollMode="never" contentInsetAdjustmentBehavior="automatic" style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ gap: spacing.md, padding: spacing.lg, paddingBottom: spacing.xl }}>
      <Animated.View entering={FadeInDown.duration(220)} style={{ gap: spacing.xs }}><Text selectable style={[typography.title, { color: colors.text }]}>Get a clear view.</Text><Text selectable style={[typography.body, { color: colors.textSecondary }]}>Place your phone anywhere stable. The angle does not need to be perfect.</Text></Animated.View>
