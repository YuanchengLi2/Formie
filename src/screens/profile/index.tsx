...
export function ProfileScreen({ displayName = "Formie Athlete", email = null, subscription, capturePreferences = defaultCapturePreferences, onSaveProfile = async () => undefined, onSaveCapturePreferences = async () => undefined, onSendFeedback = () => undefined, onManageSubscription = () => undefined, termsUrl, privacyUrl, onOpenUrl = async () => undefined, onLogOut = async () => undefined, showTestControls = false, testRemaining: currentTestRemaining = null, onTestControl = async () => undefined, onSetTestRemaining = async () => undefined }: {
  ...
  return <ScrollView alwaysBounceVertical bounces overScrollMode="auto" contentInsetAdjustmentBehavior="automatic" style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ gap: spacing.xl, padding: spacing.lg, paddingBottom: spacing.xxl }}>
    ...
    <Section title="Subscription"><Pressable accessibilityRole="button" accessibilityLabel="Manage subscription" onPress={onManageSubscription} style={{ minHeight: 52, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}><View style={{ flex: 1, gap: 3 }}><Text selectable style={[typography.heading, { color: colors.text }]}>{subscription?.plan ?? "Manage subscription"}</Text>{subscription ? <Text selectable style={[typography.caption, { color: colors.textMuted }]}>{subscription.stateLabel}</Text> : null}</View><Text style={{ color: colors.gold, fontSize: 22 }}>›</Text></Pressable></Section>
