...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return <View style={{ gap: spacing.sm }}><Text selectable style={[typography.caption, { color: colors.textMuted, letterSpacing: 0.8, textTransform: "uppercase" }]}>{title}</Text><View style={{ gap: spacing.md, borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.border, paddingVertical: spacing.md }}>{children}</View></View>;
}
...
function InfoRow({ icon, title, detail }: { icon: ProductionIconName; title: string; detail: string }) {
  return <View style={{ minHeight: 58, flexDirection: "row", alignItems: "center", gap: spacing.md }}><ProductionIcon name={icon} label={title} size={28} tintColor={colors.textSecondary} /><View style={{ flex: 1, gap: 2 }}><Text selectable style={[typography.label, { color: colors.text }]}>{title}</Text><Text selectable style={[typography.caption, { color: colors.textMuted }]}>{detail}</Text></View></View>;
}

export function ProfileScreen({ displayName = "Formie Athlete", capturePreferences = defaultCapturePreferences, onSaveProfile = async () => undefined, onSaveCapturePreferences = async () => undefined, onSendFeedback = () => undefined, onManageSubscription = () => undefined, termsUrl, privacyUrl, onOpenUrl = async () => undefined, onLogOut = async () => undefined }: {
  displayName?: string;
  ...
  return <ScrollView alwaysBounceVertical bounces overScrollMode="auto" contentInsetAdjustmentBehavior="automatic" style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ gap: spacing.xl, padding: spacing.lg, paddingBottom: spacing.xxl }}>
    <FormWordmark />
    <View style={{ gap: spacing.xs }}><Text selectable style={[typography.title, { color: colors.text }]}>Settings</Text><Text selectable style={[typography.body, { color: colors.textSecondary }]}>Your athlete profile, camera, and privacy preferences.</Text></View>
    <Section title="Athlete"><View style={{ flexDirection: "row", alignItems: "center", gap: spacing.md }}><View style={{ width: 52, height: 52, alignItems: "center", justifyContent: "center", borderRadius: 26, borderWidth: 1, borderColor: colors.gold }}><Text selectable style={{ color: colors.gold, fontSize: 20 }}>{name.slice(0, 1).toUpperCase()}</Text></View><View style={{ flex: 1, gap: 2 }}><Text selectable numberOfLines={1} style={[typography.heading, { color: colors.text }]}>{name}</Text></View></View><TextInput accessibilityLabel="Display name" autoCapitalize="words" maxLength={60} value={name} onChangeText={setName} style={[typography.body, { minHeight: 50, paddingHorizontal: spacing.md, borderRadius: radii.md, borderWidth: 1, borderColor: colors.border, color: colors.text }]} /></Section>
    ...
    <Section title="Privacy and retention"><InfoRow icon="privacyLock" title="Saved on this device" detail="Your recording stays in Formie's private device storage" /><InfoRow icon="videoStorage" title="No cloud video library" detail="Analysis uploads are removed after processing" /></Section>
    <Section title="Support"><InfoRow icon="info" title="Premium support" detail="Help with product and recording issues." /><Pressable accessibilityRole="button" onPress={onSendFeedback} style={{ minHeight: 50, justifyContent: "center" }}><Text selectable style={[typography.label, { color: colors.gold }]}>Send Feedback</Text></Pressable></Section>
    <Section title="Subscription"><Pressable accessibilityRole="button" accessibilityLabel="Manage subscription" onPress={onManageSubscription} style={{ minHeight: 50, justifyContent: "center" }}><Text selectable style={[typography.label, { color: colors.gold }]}>Manage subscription</Text></Pressable></Section>
    {termsUrl || privacyUrl ? <Section title="Legal">{termsUrl ? <Pressable accessibilityRole="link" onPress={() => void onOpenUrl(termsUrl)} style={{ minHeight: 50, justifyContent: "center" }}><Text selectable style={[typography.label, { color: colors.gold }]}>Terms of Use</Text></Pressable> : null}{privacyUrl ? <Pressable accessibilityRole="link" onPress={() => void onOpenUrl(privacyUrl)} style={{ minHeight: 50, justifyContent: "center" }}><Text selectable style={[typography.label, { color: colors.gold }]}>Privacy Policy</Text></Pressable> : null}</Section> : null}
    ...
    <View style={{ flexDirection: "row", gap: spacing.md }}><Pressable accessibilityRole="button" disabled={Boolean(busy)} onPress={reset} style={({ pressed }) => ({ flex: 1, minHeight: 52, alignItems: "center", justifyContent: "center", borderRadius: radii.md, borderWidth: 1, borderColor: colors.border, opacity: pressed || busy ? 0.6 : 1 })}><Text selectable style={[typography.label, { color: colors.textSecondary }]}>Reset Settings</Text></Pressable><FormButton style={{ flex: 1 }} label={busy === "apply" ? "Applying…" : "Apply Settings"} disabled={Boolean(busy) || name.trim().length < 2} onPress={() => void apply()} /></View>
    <Pressable testID="logout-button" accessibilityLabel="Log Out" accessibilityRole="button" disabled={Boolean(busy)} onPress={() => void logOut()} style={({ pressed }) => ({ minHeight: 52, alignItems: "center", justifyContent: "center", borderRadius: radii.md, backgroundColor: colors.danger, opacity: pressed || busy ? 0.7 : 1 })}><Text selectable style={[typography.label, { color: colors.text }]}>{busy === "logout" ? "Logging Out…" : "Log Out"}</Text></Pressable>
