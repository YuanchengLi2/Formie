...
describe("account access screens", () => {
  it("matches the approved white account-access reference on login", async () => {
    ...
    const screen = await render(withSafeArea(<SocialLoginScreen onApple={onApple} onEmailPassword={onEmailPassword} onCreateAccount={onCreateAccount} busyProvider={null} />));
  ...
  it("keeps password access separate from account creation", async () => {
    const onApple = jest.fn();
    const screen = await render(withSafeArea(<SocialLoginScreen onApple={onApple} onEmailPassword={jest.fn()} onCreateAccount={jest.fn()} busyProvider={null} />));
