...
export function AnalysisFigure() {
  ...
  const scanStyle = useAnimatedStyle(() => ({
    opacity: interpolate(scan.value, [0, 0.5, 1], [0.2, 1, 0.2]),
    transform: [{ translateY: interpolate(scan.value, [0, 1], [-72, 72]) }],
  }));
  const jointStyle = useAnimatedStyle(() => ({
    opacity: interpolate(scan.value, [0, 0.5, 1], [0.35, 1, 0.35]),
    transform: [{ scale: interpolate(scan.value, [0, 0.5, 1], [0.82, 1.12, 0.82]) }],
  }));
