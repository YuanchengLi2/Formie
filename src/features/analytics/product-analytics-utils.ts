export function safeAnalyticsProperties(properties: Record<string, unknown>): Record<string, string | number | boolean | null> {
  const allowed = new Set(["screenId", "step", "onboardingVersion", "tab", "offerId", "errorCategory", "platform", "appVersion", "buildNumber", "exerciseId", "outcome", "durationMs", "reason", "source"]);
