...
export async function reconcileWithDeadline<T>(operation: () => Promise<T>, expected: (value: T) => boolean, delays = [0, 500, 1_500, 3_500, 7_000], wait = (milliseconds: number) => new Promise<void>((resolve) => setTimeout(resolve, milliseconds))): Promise<ReconciliationResult<T>> {
  let attempts = 0;
