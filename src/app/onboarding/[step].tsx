...
export default function OnboardingStepRoute() {
  const { step: rawStep } = useLocalSearchParams<{ step?: string }>(); const router = useRouter(); const auth = useAuth(); const onboarding = useOnboarding(); const billing = useBilling(); const profile = useProfile();
  ...
  const next = nextOnboardingStep(step); const previous = previousOnboardingStep(step);
  const nextAction = () => { if (next) void go(next); };
  return <ApprovedOnboardingScreen step={step} answers={onboarding.answers}
