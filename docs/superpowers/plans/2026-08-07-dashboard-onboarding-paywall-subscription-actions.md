...
**Implementation status:** Tasks 1–9 are implemented. Focused mobile, related entitlement/quota/analytics, website tests, mobile and website typechecks/lint, and the website production build pass. Metro is restarted and its phone-facing iOS manifest plus exact LAN launch bundle return HTTP 200 with the new markers. Task 10 is complete for local verification, staging, commit, and push; the `useformie` Vercel project is now connected to `YuanchengLi2/Formie` so the pushed branch can deploy through Git, and no physical phone is connected for screenshot evidence.
